...
# 2. Fetch Academies (GID 1376867691)
...
# 3. Fetch Apartments (GID 642130592)
...
# 4. Fetch Community Centers / Dongs (GID 600107051)
...
# Fast calculation with bounding box pre-filtering
matched_results = []
LAT_DEG = 0.03   # ~3.3km
LNG_DEG = 0.04   # ~3.5km
    ...
    for a in academies:
        if abs(a['lat'] - d_lat) <= LAT_DEG and abs(a['lng'] - d_lng) <= LNG_DEG:
            if haversine(d_lat, d_lng, a['lat'], a['lng']) <= 3000:
                ...
                if academies_3km >= 100: # Early exit if 100 or more
                    break
...
print(f"\n=======================================================")
print(f"🎯 MATCHED CANDIDATE DONGS ({len(matched_results)} locations found)")
print(f"Conditions: Exclude Seoul, 3km Academies < 100, 3km Apt Families >= 30,000, 3km Students >= 10,000")
print(f"=======================================================")
